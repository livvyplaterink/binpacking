...
#orders unfilled bins by non-decreasing cost
def partition_4(cost_dict, bin_list, start, end):
    pivot = cost_dict[bin_list[start]]
    low = start + 1
    high = end
    while True:
        while low <= high and cost_dict[bin_list[high]] >= pivot:
            high = high -1
        while low <= high and cost_dict[bin_list[low]] <= pivot:
            low = low + 1
        if low <= high:
            bin_list[low], bin_list[high] = bin_list[high], bin_list[low]
        else:
...
def quick_sort_4(cost_dict, bin_list, start, end):
    if start >= end:
        return
    p = partition_4(cost_dict, bin_list, start, end)
    quick_sort_4(cost_dict, bin_list, start, p-1)
    quick_sort_4(cost_dict, bin_list, p+1, end)
...
quick_sort_4(bin_cost, unfilled_bins, 0, len(unfilled_bins) -1)
...
#quick_sort_2(cv_ratio, bin_volume, ordered_bins, 0, len(ordered_bins)-1)
#print(ordered_bins) 
#quick_sort_3(cv_ratio, bin_volume, ordered_bins, 0, len(ordered_bins)-1)
#print(ordered_bins)
#quick_sort_4(bin_cost, ordered_bins, 0, len(ordered_bins)-1)
#print(ordered_bins)
